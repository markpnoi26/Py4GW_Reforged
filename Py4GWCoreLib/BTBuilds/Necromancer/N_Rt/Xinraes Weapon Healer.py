...
class Xinraes_Weapon_Healer(BldMgrBT):
    ...
    def build_rotation_tree(self) -> BehaviorTree:
        ...
        return rotation_tree(
            ...
            [
                cast(self, "SpiritTransferEmergency",
                     lambda: resto().Spirit_Transfer(health_threshold=0.30)),
                cast(self, "SpiritLightEmergency",
                     lambda: resto().Spirit_Light(health_threshold=0.30)),
                cast(self, "MendBodyAndSoul40",
                     lambda: resto().Mend_Body_and_Soul(health_threshold=0.40)),
                guarded_cast(self, "DropHeldBundle", equipped(Protective_Was_Kaolai_ID),
                             lambda: self.skills.Ritualist.NoAttribute.Drop_Held_Bundle(health_threshold=0.75)),
                cast(self, "SignetOfLostSouls30",
                     lambda: necro().SoulReaping.Signet_of_Lost_Souls(max_self_energy_pct=0.30)),
                guarded_cast(self, "RecuperationDamaged6a", equipped(Recuperation_ID),
                             lambda: resto().Recuperation(min_party_damaged_count=6)),
                guarded_cast(self, "RecuperationDegen6", equipped(Recuperation_ID),
                             lambda: resto().Recuperation(min_degen_count=6)),
                guarded_cast(
                    self, "AirOfSuperiority",
                    lambda: self.IsSkillEquipped(Air_of_Superiority_ID)
                    and (self.IsInAggro() or self.IsCloseToAggro()),
                    lambda: anyskills().PvE.Air_of_Superiority(),
                ),
                guarded_cast(self, "WieldersBoon", equipped(Wielders_Boon_ID),
                             lambda: resto().Wielders_Boon()),
                guarded_cast(self, "MendingGrip", equipped(Mending_Grip_ID),
                             lambda: resto().Mending_Grip()),
                guarded_cast(self, "SpiritTransfer", equipped(Spirit_Transfer_ID),
                             lambda: resto().Spirit_Transfer()),
                cast(self, "MendBodyAndSoulCleanseBlind",
                     lambda: resto().Mend_Body_and_Soul(cleanse_blind_martial=True)),
                cast(self, "MendBodyAndSoulCleanseCripple",
                     lambda: resto().Mend_Body_and_Soul(cleanse_cripple_melee=True)),
                guarded_cast(self, "RecuperationDamaged6b", equipped(Recuperation_ID),
                             lambda: resto().Recuperation(min_party_damaged_count=6)),
                guarded_cast(self, "RecuperationDegen4", equipped(Recuperation_ID),
                             lambda: resto().Recuperation(min_degen_count=4)),
                cast(self, "SignetOfLostSouls60",
                ...
                cast(self, "MendBodyAndSoul75",
                     lambda: resto().Mend_Body_and_Soul(health_threshold=0.75)),
                guarded_cast(self, "ProtectiveWasKaolai", equipped(Protective_Was_Kaolai_ID),
                ...
                sequence(
                    ...
                    selector(
                        "AggroRungs",
                        guarded_cast(self, "WeakenArmor", equipped(Weaken_Armor_ID),
                                     lambda: necro().Curses.Weaken_Armor()),
                        ...
                        guarded_cast(self, "YouAreAllWeaklings", equipped(You_Are_All_Weaklings_ID),
                                     lambda: anyskills().NoAttribute.You_Are_All_Weaklings()),
                        cast(self, "SpiritLight", lambda: resto().Spirit_Light()),
                        guarded_cast(self, "VitalWeapon", equipped(Vital_Weapon_ID),
                                     lambda: self.skills.Ritualist.Communing.Vital_Weapon()),
                        guarded_cast(self, "Recovery", equipped(Recovery_ID),
                                     lambda: resto().Recovery()),
                        cast(self, "SignetOfLostSouls",
                             lambda: necro().SoulReaping.Signet_of_Lost_Souls()),
                        guarded_cast(self, "RecuperationDamaged6c", equipped(Recuperation_ID),
                                     lambda: resto().Recuperation(min_party_damaged_count=6)),
                        guarded_cast(self, "RecuperationDegen2", equipped(Recuperation_ID),
                                     lambda: resto().Recuperation(min_degen_count=2)),
                        cast(self, "MendBodyAndSoul85",
                             lambda: resto().Mend_Body_and_Soul(health_threshold=0.85)),
                        guarded_cast(self, "BreathOfTheGreatDwarf",
                                     equipped(Breath_of_the_Great_Dwarf_ID),
                                     lambda: anyskills().NoAttribute.Breath_of_the_Great_Dwarf()),
                        guarded_cast(self, "EnfeeblingBlood", equipped(Enfeebling_Blood_ID),
