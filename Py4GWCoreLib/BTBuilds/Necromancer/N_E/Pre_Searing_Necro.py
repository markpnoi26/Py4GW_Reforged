...
from Py4GWCoreLib import BldMgrBT, GLOBAL_CACHE, Player, Profession, Range, Routines
...
class Pre_Searing_Necro(BldMgrBT):
    ...
    def sample_enemy_count(self, node) -> bool:
        ...
        nearby_enemies = Routines.Agents.GetFilteredEnemyArray(
            player_x, player_y, max_distance=Range.Spellcast.value)
    ...
    def build_rotation_tree(self) -> BehaviorTree:
        ...
        return rotation_tree(
            ...
            [
                guarded_cast(self, "ResurrectionSignet", equipped(Resurrection_Signet_ID),
                             lambda: self.resurrection_signet()),
                guarded_cast(
                    self, "AuraOfRestoration",
                    ...
                    lambda: self.CastSkillID(
                        skill_id=Aura_of_Restoration_ID, log=False, aftercast_delay=250),
                ),
                sequence(
                    "InAggroRotation",
                    ...
                    selector(
                        ...
                        guarded_cast(
                            self, "FireStorm",
                            lambda node: self.IsSkillEquipped(Fire_Storm_ID)
                            and self.enemy_count(node) >= 3,
                            lambda: self.fire_storm(),
                        ),
                        guarded_cast(
                            self, "DeathlySwarm",
                            lambda node: self.IsSkillEquipped(Deathly_Swarm_ID)
                            and self.enemy_count(node) >= 2,
                            lambda: self.cast_at_nearest_enemy(Deathly_Swarm_ID),
                        ),
                        guarded_cast(self, "Flare", equipped(Flare_ID),
                                     lambda: self.cast_at_nearest_enemy(Flare_ID)),
