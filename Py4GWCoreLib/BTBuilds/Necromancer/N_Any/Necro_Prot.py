...
from Py4GWCoreLib import BldMgrBT, Profession, Range, Routines
    ...
    def build_rotation_tree(self) -> BehaviorTree:
        ...
        return rotation_tree(
            ...
            [
                ...
                cast(self, "ProtectiveSpirit",
                     lambda: prot().Protective_Spirit(prebuff_melee_precombat=True)),
                guarded_cast(self, "ShieldOfAbsorption", equipped(SHIELD_OF_ABSORPTION_ID),
                             lambda: prot().Shield_of_Absorption()),
                guarded_cast(self, "AuraOfFaith", equipped(AURA_OF_FAITH_ID),
                             lambda: prot().Aura_of_Faith()),
                guarded_cast(self, "LifeSheath", equipped(LIFE_SHEATH_ID),
                             lambda: prot().Life_Sheath(min_conditions=2)),
                guarded_cast(self, "DivertHexes", equipped(DIVERT_HEXES_ID),
                             lambda: prot().Divert_Hexes(min_hexes=2)),
                cast(self, "ReverseHex", lambda: prot().Reverse_Hex()),
                guarded_cast(self, "ReversalOfFortune", equipped(REVERSAL_OF_FORTUNE_ID),
                             lambda: prot().Reversal_of_Fortune()),
            ],
        )
