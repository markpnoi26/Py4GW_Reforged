...
class VoS_Grenths_Aura_Farmer(BldMgrBT):
    ...
    def get_snapshot(self) -> VoSSnapshot:
        ...
        return VoSSnapshot(
            ...
            has_battle_standard=Routines.Checks.Effects.HasBuff(
                player_agent_id, Ebon_Battle_Standard_of_Honor_ID),
            mirage_remaining=(
                GLOBAL_CACHE.Effects.GetEffectTimeRemaining(player_agent_id, Mirage_Cloak_ID)
                if has_mirage_cloak else 0
            ),
        )
    ...
    def deaths_charge(self):
        ...
        return (yield from self.CastSkillIDAndRestoreTarget(
            Deaths_Charge_ID, target, aftercast_delay=350))

    def build_rotation_tree(self) -> BehaviorTree:
        simple_cast = lambda skill_id, delay=250: (lambda: self.CastSkillID(
            skill_id, aftercast_delay=delay))
        return rotation_tree(
            "VoSGrenthsAuraFarmer",
            [],
            [
                guarded_cast(
                    self, "VowOfStrengthPrecombat",
                    lambda node: not self.snapshot(node).in_combat
                    and not self.snapshot(node).has_vow_of_strength,
                    simple_cast(Vow_of_Strength_ID),
                ),
                guarded_cast(
                    self, "MysticRegenerationPrecombat",
                    lambda node: not self.snapshot(node).in_combat
                    and not self.snapshot(node).has_mystic_regeneration,
                ...
                guarded_cast(
                    self, "EbonBattleStandardOfHonor",
                    lambda node: self.snapshot(node).in_combat
                    and self.snapshot(node).nearby_enemy_count >= 3
                    and not self.snapshot(node).has_battle_standard,
                    lambda: self.CastSpiritSkillID(
                        Ebon_Battle_Standard_of_Honor_ID, aftercast_delay=250),
                ),
                guarded_cast(
                    self, "IAmUnstoppable",
                    lambda node: self.snapshot(node).in_combat
                    and not self.snapshot(node).has_i_am_unstoppable,
                ...
                guarded_cast(
                    self, "MirageCloak",
                    lambda node: self.snapshot(node).in_combat
                    and (not self.snapshot(node).has_mirage_cloak
                         or self.snapshot(node).mirage_remaining <= 1500),
                    simple_cast(Mirage_Cloak_ID),
                ),
                guarded_cast(
                    self, "GrenthsAuraSpike",
