...
class Tao_Dagger_Spam(BldMgrBT):
    ...
    def build_rotation_tree(self) -> BehaviorTree:
        ...
        return rotation_tree(
            ...
            [
                guarded_cast(
                    self, "IAmTheStrongest", equipped(I_Am_the_Strongest_ID),
                    lambda: self.CastSkillID(
                        skill_id=I_Am_the_Strongest_ID, log=False, aftercast_delay=250),
                ),
                guarded_cast(
                    self, "ComfortAnimal",
                ...
                guarded_cast(
                    self, "BreathOfTheGreatDwarf", equipped(Breath_of_the_Great_Dwarf_ID),
                ...
                sequence(
                    ...
                    selector(
                        "AggroRungs",
                        guarded_cast(
                            self, "LightningReflexes", equipped(Lightning_Reflexes_ID),
                            lambda: self.CastSkillID(
                                skill_id=Lightning_Reflexes_ID, log=False, aftercast_delay=250),
                        ),
                        cast(self, "TogetherAsOne",
                             lambda: self.skills.Ranger.Expertise.Together_as_One()),
