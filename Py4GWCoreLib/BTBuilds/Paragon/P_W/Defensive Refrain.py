...
class Paragon_Refrain(BldMgrBT):
    ...
    def build_rotation_tree(self) -> BehaviorTree:
        ...
        return rotation_tree(
            ...
            [
                guarded_cast(self, "HeroicRefrain", equipped(Heroic_Refrain_ID),
                             lambda: paragon().Leadership.Heroic_Refrain()),
                guarded_cast(self, "TheyreOnFire", equipped(Theyre_on_Fire_ID),
                             lambda: paragon().Leadership.Theyre_on_Fire()),
                sequence(
                    "InAggroRotation",
                    cond("InAggro", lambda: self.IsInAggro()),
                    selector(
                        "AggroRungs",
                        guarded_cast(self, "AngelicProtection", equipped(Angelic_Protection_ID),
                                     lambda: paragon().Leadership.Angelic_Protection(health_threshold=0.30)),
                        guarded_cast(self, "TheresNothingToFear", equipped(Theres_Nothing_to_Fear_ID),
                                     lambda: anyskills().NoAttribute.Theres_Nothing_to_Fear()),
                        guarded_cast(self, "AggressiveRefrain", equipped(Aggressive_Refrain_ID),
                                     lambda: paragon().Leadership.Aggressive_Refrain()),
                        guarded_cast(self, "ForGreatJustice", equipped(For_Great_Justice_ID),
                                     lambda: self.skills.Warrior.NoAttribute.For_Great_Justice()),
                        guarded_cast(self, "MakeYourTime", equipped(Make_Your_Time_ID),
                                     lambda: paragon().Leadership.Make_Your_Time()),
                        guarded_cast(self, "SaveYourselvesLuxon", equipped(Save_Yourselves_luxon_ID),
                                     lambda: anyskills().NoAttribute.Save_Yourselves_luxon()),
                        guarded_cast(self, "SaveYourselvesKurzick", equipped(Save_Yourselves_kurzick_ID),
                                     lambda: anyskills().NoAttribute.Save_Yourselves_kurzick()),
                        guarded_cast(self, "StandYourGround", equipped(Stand_Your_Ground_ID),
                                     lambda: paragon().Command.Stand_Your_Ground()),
                        guarded_cast(self, "CantTouchThis", equipped(Cant_Touch_This_ID),
                                     lambda: paragon().Command.Cant_Touch_This()),
                        guarded_cast(self, "HastyRefrain", equipped(Hasty_Refrain_ID),
                                     lambda: paragon().Motivation.Hasty_Refrain()),
                        guarded_cast(self, "NeverSurrender", equipped(Never_Surrender_ID),
                                     lambda: paragon().Motivation.Never_Surrender()),
                        guarded_cast(self, "BlazingFinale", equipped(Blazing_Finale_ID),
                                     lambda: paragon().Motivation.Blazing_Finale()),
                        guarded_cast(self, "ProtectorsDefense", equipped(Protectors_Defense_ID),
                        ...
                        guarded_cast(self, "EbonBattleStandardOfWisdom",
