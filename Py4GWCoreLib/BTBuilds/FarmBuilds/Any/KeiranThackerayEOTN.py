...
from Py4GWCoreLib import (GLOBAL_CACHE, ActionQueueManager, Agent, AgentArray, AutoPathing,
                          BldMgrBT, ConsoleLog, Map, Player, Range, Routines)
...
def escape_point(me_x: float, me_y: float, threat_x: float, threat_y: float,
                 dist: float, rotation: int = 0, debug_fn=None):
...
class KeiranThackerayEOTN(BldMgrBT):
    ...
    def escort_routine(self):
        ...
        if miku_id != 0 and not miku_dead:
            ...
            self.miku_idle = (
                Agent.IsIdle(miku_id)
                and not Agent.IsInCombatStance(miku_id)
                and len(enemies_near_miku) == 0
            )
        else:
        ...
        if miku_reset:
            ...
            if self.miku_reset_at == 0.0:
                ...
                nearest_idx = min(range(len(MIKU_PATH)),
                                  key=lambda i: distance_between(player_x, player_y, *MIKU_PATH[i]))
        ...
        if Routines.Checks.Player.CanAct():
            ...
            damage_dealt = any(
                src == player_id and (tick_now - ts) < LOS_WINDOW_MS
                for ts, tgt, src, _dmg, _skill, _crit in los_recent
            )
            damage_received = any(
                tgt == player_id and (tick_now - ts) < LOS_WINDOW_MS
                for ts, tgt, src, _dmg, _skill, _crit in los_recent
            )
                ...
                if self.miku_lazy_at == 0.0:
                    ...
                    if nearest_enemy != 0:
                        ...
                        if len(enemies_far) > 1:
                            ...
                            ex_x, ex_y = escape_point(player_x, player_y, ne_x, ne_y, 300, rotation=180, debug_fn=self.debug_fn)
                        ActionQueueManager().ResetAllQueues()
                        self.last_movement_run = now
                        self.combat_approach_at = 0.0
                        self.miku_lazy_at = 0.0
            ...
            if (self.player_combat and self.combat_approach_at != 0.0 and
                ...
                priority_valid = (self.locked_target_id != 0 and
                ...
                if priority_valid:
                    if damage_dealt:
                        ...
                        ActionQueueManager().ResetAllQueues()
                        ep_x, ep_y = escape_point(player_x, player_y, pl_x, pl_y, 300, rotation=180, debug_fn=self.debug_fn)
                        self.last_movement_run = now
                        self.combat_approach_at = 0.0
                    ...
                    if damage_dealt or damage_received:
                        ...
                        if move_target != 0:
                            ...
                            ep_x, ep_y = escape_point(player_x, player_y, ne_x, ne_y, 300, rotation=180, debug_fn=self.debug_fn)
                            self.last_movement_run = now
                            self.combat_approach_at = 0.0
                            Player.Move(ep_x, ep_y)
                            yield from Routines.Yield.wait(500)
            ...
            if self.aoe_caster_id != 0 and now >= self.aoe_sidestep_at:
                ...
                if tx != 0.0 or ty != 0.0:
                    ...
                    sx, sy = escape_point(player_x, player_y, tx, ty, AOE_SIDESTEP_DIST, rotation=90, debug_fn=self.debug_fn)
                    ActionQueueManager().ResetAllQueues()
                    Player.Move(sx, sy)
                    yield from Routines.Yield.wait(500)
                    self.last_movement_run = now
        ...
        if not has_empathy and now - self.last_target_check >= TARGET_SWITCH_INTERVAL:
            ...
            if self.locked_target_id != 0:
                ...
                if (not Agent.IsValid(self.locked_target_id) or
                        ...
                        distance_between(player_x, player_y, pl_x, pl_y) > PRIORITY_TARGET_RANGE):
                    self.locked_target_id = 0
                    self.locked_priority = len(PRIORITY_TARGET_MODELS)
                    self.los_fail_since = 0.0
        ...
        if in_danger:
            ...
            if terminal_velocity_ready:
                if not has_empathy:
                    target = (self.locked_target_id or
                              Routines.Targeting.GetEnemyCasting(WEAPON_RANGE.value) or
                              Routines.Targeting.GetEnemyBleeding(WEAPON_RANGE.value))
                    if target != 0:
                        self.last_cast_at = now
            ...
            if gravestone_marker_ready:
                if not has_empathy:
                    target = (self.locked_target_id or
                              Routines.Targeting.GetNearestSpirit(WEAPON_RANGE.value) or
                              Routines.Targeting.GetEnemyHealthy(WEAPON_RANGE.value))
                    if target != 0:
                        self.last_cast_at = now
            ...
            if rain_of_arrows_ready:
                if not has_empathy:
                    target = (self.locked_target_id or
                              Routines.Targeting.GetNearestSpirit(WEAPON_RANGE.value) or
                              Routines.Targeting.TargetClusteredEnemy(WEAPON_RANGE.value))
                    if target != 0:
                        self.last_cast_at = now
