...
class AssassinShadowTheftDaggerSpammer(BldMgrBT):
    ...
    def find_shadow_theft_target(self):
        ...
        enemy_agent_ids = Routines.Agents.GetFilteredEnemyArray(
            player_x, player_y, Range.Earshot.value * 0.5)
        ...
        if self.status == BuildStatus.Kill:
            ...
            if Routines.Checks.Player.CanAct() and Routines.Checks.Skills.CanCast():
                ...
                if dist_sq <= Range.Adjacent.value ** 2:
                    ...
                    if (yield from Routines.Yield.Skills.IsSkillIDUsable(self.exhausting_assault)) and player_current_energy >= 10:
                        nearest_enemy_agent = Agent.GetAgentByID(nearest_enemy_agent_id)
                    ...
                    if (yield from Routines.Yield.Skills.IsSkillIDUsable(self.death_blossom)) and player_current_energy >= 12:
                        player_x, player_y = Player.GetXY()
