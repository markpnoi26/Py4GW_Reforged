...
class SF_Ass_vaettir(BldMgrBT):
    def __init__(self, match_only: bool = False):
        ...
        self.wastrels_demise_slot = GLOBAL_CACHE.SkillBar.GetSlotBySkillID(
            GLOBAL_CACHE.Skill.GetID("Wastrels_Demise"))
        self.arcane_echo_slot = GLOBAL_CACHE.SkillBar.GetSlotBySkillID(
            GLOBAL_CACHE.Skill.GetID("Arcane_Echo"))
    ...
    def cast_skill_id(self, skill_id: int, extra_condition: bool = True,
                      log: bool = True, aftercast_delay: int = 1000):
    ...
    def cast_skill_slot(self, slot: int, extra_condition: bool = True,
                        log: bool = True, aftercast_delay: int = 1000):
    ...
    def DefensiveActions(self):
        ...
        if (yield from Routines.Yield.Skills.IsSkillIDUsable(self.shadow_form)):
            if (yield from self.cast_skill_id(self.deadly_paradox, extra_condition=(not has_deadly_paradox), log=False, aftercast_delay=100)):
                ConsoleLog(self.build_name, "Casting Deadly Paradox.", PySystem.Console.MessageType.Info, log=False)
            if (yield from self.cast_skill_id(self.shadow_form, extra_condition=(has_deadly_paradox), log=False, aftercast_delay=1750)):
    ...
    def UpkeepShroudOfDistress(self, min_remaining_buff_duration: int = 3000):
        ...
        has_shroud_of_distress = (
            ...
            and GLOBAL_CACHE.Effects.GetEffectTimeRemaining(player_agent_id, self.shroud_of_distress) > min_remaining_buff_duration
        )
    ...
    def farm_routine(self):
        ...
        if not Map.GetMapID() == Map.GetMapIDByName("Jaga Moraine"):
            ...
            agent_array = AgentArray.Filter.ByCondition(
                agent_array,
                lambda agent: Agent.GetModelID(agent) in (
                    AgentModelID.FROZEN_ELEMENTAL.value, AgentModelID.FROST_WURM.value),
            )
        ...
        shadow_form_buff_time_remaining = (
            GLOBAL_CACHE.Effects.GetEffectTimeRemaining(player_agent_id, self.shadow_form)
            if has_shadow_form else 0
        )
        ...
        if Routines.Checks.Agents.InDanger(Range.Spellcast):
            ...
            if (yield from Routines.Yield.Skills.IsSkillIDUsable(self.shadow_form)):
                ...
                if (yield from self.cast_skill_id(self.deadly_paradox, extra_condition=(not has_deadly_paradox), log=False, aftercast_delay=200)):
                    ConsoleLog(self.build_name, "Casting Deadly Paradox.", PySystem.Console.MessageType.Info, log=False)
        ...
        has_channeling = (
            ...
            and GLOBAL_CACHE.Effects.GetEffectTimeRemaining(player_agent_id, self.channeling) > min_remaining_buff_duration
        )
        ...
        if self.in_killing_routine and has_shadow_form and has_shroud_of_distress and has_channeling:
            both_ready = (
                Routines.Checks.Skills.IsSkillSlotReady(self.wastrels_demise_slot)
            ...
            if target and shadow_form_buff_time_remaining >= 5000:
                ...
                if (yield from self.cast_skill_slot(self.arcane_echo_slot, extra_condition=both_ready, log=False, aftercast_delay=2750)):
