...
class KeystoneSignet(BldMgrBT):
    ...
    def get_bar_snapshot(self) -> KeystoneBarSnapshot:
        ...
        snapshot.attacking_enemy_in_spellcast = bool(
            Routines.Targeting.GetEnemyAttacking(Range.Spellcast.value))
        return snapshot
    ...
    def build_rotation_tree(self) -> BehaviorTree:
        ...
        return rotation_tree(
            ...
            [
                guarded_cast(
                    self, "CryOfFrustration",
                    lambda node: self.snapshot(node).enemy_casting,
                    lambda: self.skills.Mesmer.DominationMagic.Cry_of_Frustration(),
                ),
                guarded_cast(self, "AnimateFleshGolem", equipped(Animate_Flesh_Golem_ID),
                             lambda: death().Animate_Flesh_Golem()),
                guarded_cast(self, "AnimateBoneFiend", equipped(Animate_Bone_Fiend_ID),
                             lambda: death().Animate_Bone_Fiend()),
                guarded_cast(self, "AnimateBoneHorror", equipped(Animate_Bone_Horror_ID),
                             lambda: death().Animate_Bone_Horror()),
                guarded_cast(self, "AnimateBoneMinions", equipped(Animate_Bone_Minions_ID),
                             lambda: death().Animate_Bone_Minions()),
                guarded_cast(self, "AnimateShamblingHorror", equipped(Animate_Shambling_Horror_ID),
                             lambda: death().Animate_Shambling_Horror()),
                guarded_cast(self, "AnimateVampiricHorror", equipped(Animate_Vampiric_Horror_ID),
                             lambda: death().Animate_Vampiric_Horror()),
                cast(self, "SmiteHexHigh",
                     lambda: smiting().Smite_Hex(min_priority=HexRemovalPriority.HIGH)),
                guarded_cast(
                    self, "SymbolicCelerity",
                    lambda node: self.snapshot(node).symbolic_celerity_needed,
                    lambda: fast_casting().Symbolic_Celerity(),
                ),
                guarded_cast(self, "HexEaterSignet", equipped(Hex_Eater_Signet_ID),
                             lambda: self.skills.Mesmer.InspirationMagic.Hex_Eater_Signet()),
                guarded_cast(self, "BreathOfTheGreatDwarf", equipped(Breath_of_the_Great_Dwarf_ID),
                             lambda: self.skills.Any.NoAttribute.Breath_of_the_Great_Dwarf()),
                guarded_cast(self, "BloodRitual", equipped(Blood_Ritual_ID),
                ...
                sequence(
                    ...
                    selector(
                        "AggroRungs",
                        guarded_cast(self, "DeathNova", equipped(Death_Nova_ID),
                                     lambda: death().Death_Nova()),
                        guarded_cast(
                            self, "KeystoneSignet",
                            lambda node: self.snapshot(node).keystone_signet_needed,
                            lambda: fast_casting().Keystone_Signet(),
                        ),
                        guarded_cast(self, "TryptophanSignet", equipped(Tryptophan_Signet_ID),
