...
from Py4GWCoreLib import Agent, BldMgrBT, Player, Profession, Range, Routines
...
class Psychic_Instability_Wastrels(BldMgrBT):
    ...
    def pick_demise(self, **kwargs) -> int:
        ...
        self.wastrels_demise_last_cast = {
            a: t for a, t in self.wastrels_demise_last_cast.items()
            if now - t < WASTRELS_DEMISE_COOLDOWN_S
        }
    ...
    def pick_worry(self, **kwargs) -> int:
        ...
        self.wastrels_worry_last_cast = {
            a: t for a, t in self.wastrels_worry_last_cast.items()
            if now - t < WASTRELS_WORRY_COOLDOWN_S
        }
        return pick_wastrels_target(
    ...
    def build_rotation_tree(self) -> BehaviorTree:
        ...
        return rotation_tree(
            ...
            [
                cast(self, "WastrelsDemiseKnockedDown",
                     lambda: self.cast_demise(require_knockdown=True)),
                cast(self, "WastrelsWorryKnockedDown",
                     lambda: self.cast_worry(require_knockdown=True)),
                guarded_cast(
                    self, "PowerDrainLowEnergy",
                    lambda node: self.snapshot(node).enemy_casting_spell_or_chant,
                    lambda: inspiration().Power_Drain(energy_threshold_pct=0.30),
                ),
                guarded_cast(
                    self, "PsychicInstability",
                ...
                cast(self, "WastrelsDemiseLowPriority",
                     lambda: self.cast_demise(min_energy_abs=10, exclude_knockdown=True)),
                cast(self, "WastrelsWorryLowPriority",
                     lambda: self.cast_worry(min_energy_abs=10, exclude_knockdown=True)),
            ],
        )
