...
class EnemyTrackerConfig:
    ...
    DATA_DOC_NAME: str = "EnemyTracker/Data.json"
    NAMES_DOC_NAME: str = "EnemyTracker/Names.json"
...
class EnemyTracker:
    ...
    def __init__(self) -> None:
        ...
        # Cross-account shared store. Global scope already provides a cross-process
        # lock + per-path journal-merge, so no hand-rolled lock/atomic dance is needed.
        self._data_doc = JsonFactory(EnemyTrackerConfig.DATA_DOC_NAME, "global")
        self._names_doc = JsonFactory(EnemyTrackerConfig.NAMES_DOC_NAME, "global")
        if shared_vars is None or (not self.vars.records and not self.vars.name_records):
            self._load_data()

    def _load_data(self) -> None:
        try:
            enemies = self._data_doc.get_json("enemies", {}) or {}
            if isinstance(enemies, dict):
                self.vars.records = self._normalize_records(dict(enemies))
            if int(self._data_doc.get_int("schema_version", 1) or 1) < 2:
    ...
    def _load_name_data(self) -> None:
        raw_names = self._names_doc.get_json("names", {}) or {}
        if not isinstance(raw_names, dict):
            return
        for language, names_by_key in raw_names.items():
            language_key = str(language or "").strip().lower()
            if not language_key or language_key in self.vars.name_records:
                continue
            if isinstance(names_by_key, dict):
                self.vars.name_records[language_key] = self._normalize_name_records(dict(names_by_key))

    def _save_data_if_needed(self, force: bool = False) -> None:
        if not (force or self.vars.data_dirty or self.vars.names_dirty):
            return
        ...
        try:
            # Targeted per-path writes so concurrent accounts' entries survive the
            # global-scope journal-merge (per-path last-writer-wins). Whole-subtree
            # rewrites would clobber peers' keys, so every record/name is addressed
            # individually. set_json dedups unchanged subtrees, so re-writing every
            # known key on each dirty cycle is cheap.
            if self.vars.data_dirty or force:
                self._data_doc.set("schema", "py4gw_enemy_tracker")
                self._data_doc.set("schema_version", 2)
                for key, record in self.vars.records.items():
                    self._data_doc.set_json(f"enemies/{key}", record)

            if self.vars.names_dirty or force:
                self._names_doc.set("schema", "py4gw_enemy_tracker_names")
                self._names_doc.set("schema_version", 1)
                for language, names_by_key in self.vars.name_records.items():
                    for key, names in names_by_key.items():
                        self._names_doc.set_json(f"names/{language}/{key}", names)
    ...
    def draw_scanner_config(self) -> None:
        PyImGui.text(f"Data file: {self._data_doc.resolved_path()}")
        ...
        if PyImGui.button("Import / Merge JSON"):
            ...
            try:
                path = filedialog.askopenfilename(
                    ...
                    initialdir=PySystem.Console.get_projects_path(),
