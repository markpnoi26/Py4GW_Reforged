...
def junundu_siege_check() -> bool:
    return (Routines.Agents.GetNearestEnemy(Range.Nearby.value) != 0
...
def build_registry(handler) -> dict[int, Predicate]:
    ...
    registry: dict[int, Predicate] = {
        ...
        handler.essence_strike: lambda h, c, t: (
            ...
            and Routines.Agents.GetNearestSpirit(Range.Spellcast.value) != 0
        ),
        handler.glowing_signet: lambda h, c, t: (
            player_energy_below(h, c.LessEnergy) and h.HasEffect(t, h.burning)
        ),
        ...
        handler.mend_body_and_soul: lambda h, c, t: (
            ...
            or (Routines.Agents.GetNearestSpirit(Range.Earshot.value) != 0
                and Routines.Checks.Agents.IsConditioned(t))
        ),
        handler.grenths_balance: lambda h, c, t: (
            player_life_below(c.LessLife)
            and Agent.GetHealth(Player.GetAgentID()) < Routines.Checks.Agents.GetHealth(t)
        ),
        ...
        handler.junundu_tunnel: lambda h, c, t: (
            Routines.Agents.GetNearestEnemy(h.get_combat_distance()) == 0
        ),
