...
def decide_slot_native(handler, slot: int) -> tuple[bool, int]:
    ...
    if skill_type == SkillType.Shout.value:
        if handler.HasEffect(player_id, handler.vocal_minority) or handler.HasEffect(player_id, handler.well_of_silence):
            return reject()
    ...
    if skill_type == SkillType.Hex.value:
        if Agent.IsSpirit(v_target) or (
            v_target_allegiance == Allegiance.Enemy.value and Agent.IsSpawned(v_target)
        ):
            return reject()
    ...
    exact_weapon_spell = (
        skill_type == SkillType.WeaponSpell.value and conditions.AllowOverlapWeaponSpell
    )
    if (target_allegiance != Skilltarget.NonWeaponSpelledAlly.value
            and handler.HasEffect(v_target, skill_id, exact_weapon_spell=exact_weapon_spell)):
        return reject(v_target)
