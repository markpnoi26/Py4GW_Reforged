...
evidence base for `../02_implementation_spec.md` — it is the record of *which decisions leaked into
the implementation phase*, which is the failure the spec exists to prevent.

---


Every decision in `01_class.md` checked against the code. Verified by inspection and by targeted
