__script__ = {
    "name": "Shared Memory Isolation Manager",
    "function": "debug",
    "tags": [],
    "claims": ["sharedmem"],
}
...
def _load_groups(force: bool = False):
    ...
    for i in range(count):
        gid = cfg.get_int("Groups", f"id_{i}", 0)
        name = str(cfg.get_str("Groups", f"name_{i}", "") or "").strip()
        if gid > 0 and name:
            _groups[gid] = name
...
def _save_groups():
    ...
    for i, (gid, name) in enumerate(sorted(_groups.items())):
        cfg.set("Groups", f"id_{i}", gid)
        cfg.set("Groups", f"name_{i}", name)
...
def _apply_assignments():
    ...
    for account in accounts:
        ...
        if not email:
            continue
        stored_gid = cfg.get_int("Assignments", email, 0)
