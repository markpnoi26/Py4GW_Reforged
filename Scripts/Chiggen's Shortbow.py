__script__ = {
    "name": "Chiggen's Shortbow Farm",
    "function": "farmer",
    "tags": ["bow", "green", "weapon"],
    "claims": ["character"],
}
...
BOT_NAME = "Chiggen's Shortbow Farm"
...
def Routine(bot: Botting) -> None:
    """Routine principale du farm Chiggen's Shortbow"""
    ...
    bot.Templates.Routines.PrepareForFarm(map_id_to_travel=381)
    bot.Properties.Disable("auto_inventory_management")
    bot.Party.SetHardMode(False)
    # ==================================

    bot.States.AddHeader("Exit Outpost")
    bot.Move.XYAndExitMap(-5022.78, 4951.90, target_map_id=371)
    bot.Wait.ForMapLoad(371)
    bot.Wait.ForTime(3000)

    bot.States.AddHeader("Combat")
    bot.Move.FollowAutoPath(KILLING_PATH, "Chiggen's Shortbow Kill Route")
...
def tooltip():
    ...
    PyImGui.text_colored("Chiggen's Shortbow Farmer bot", title_color.to_tuple_normalized())
    ImGui_Legacy.pop_font()
    PyImGui.spacing()
    PyImGui.separator()
    # Description
    PyImGui.text("multi-account bot to farm Chiggen's Shortbow")
    PyImGui.spacing()
